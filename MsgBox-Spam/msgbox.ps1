...
[System.Windows.Forms.Application]::AddMessageFilter({
    Param([System.Windows.Forms.Message]$m)
    $WM_KEYDOWN = 0x100
    $WM_KEYUP = 0x101

    if ($m.Msg -eq $WM_KEYDOWN) { $onKeyDown.Invoke($null, [System.Windows.Forms.KeyEventArgs]::new($m.WParam)) }
    if ($m.Msg -eq $WM_KEYUP) { $onKeyUp.Invoke($null, [System.Windows.Forms.KeyEventArgs]::new($m.WParam)) }
